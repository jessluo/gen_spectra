...
# load data
ss <- read.csv("test_cases/data/phytoplankton_input_data.csv", stringsAsFactors=FALSE)
sz <- read.csv("test_cases/data/zooplankton_input_data.csv", stringsAsFactors=FALSE)
sg <- read.csv("test_cases/data/grazing_input_data.csv", stringsAsFactors=FALSE)
gf <- read.csv("test_cases/data/grazing_FK_value.csv")

dir.create("test_cases/plots", showWarnings=FALSE)

# some initial plots
ss$type <- gsub('[[:digit:]]+', '', ss$sname)
ss[which(ss$type=="pp"), "type"] <- "mp"
sz$type <- 'zoo'
...
ggsave("test_cases/plots/PFT_mass_v_ESD.png", p, height=12, width=12)
...
ggsave("test_cases/plots/PFT_growth_parameters.png", p1, height=6, width=6)
...
ggsave("test_cases/plots/PFT_stochiometry.png", p1, height=2.5, width=4.5)
...
ggsave("test_cases/plots/PFT_nutrient_parameters.png", p2, height=6, width=6)
...
ggsave("test_cases/plots/PFT_nutrient_parameters_b.png", p2b, height=6, width=6)
...
ggsave("test_cases/plots/PFT_loss_parameters.png", p3, height=6, width=6)
...
ggsave("test_cases/plots/Zoo_parameters.png", p4, height=6, width=6)
...
p5 <- ggplot(gf_melt) + geom_tile(aes(y=prey, x=pred, fill=value)) + 
	scale_fill_gradient2(high="#b2182b", low="#f7f7f7")+#, midpoint=0.3, limits=c(0,0.6)) +
	labs(x="Predators", y="Prey", fill="FK value") + scale_x_discrete(expand=c(0,0)) + scale_y_discrete(expand=c(0,0))
	
ggsave("test_cases/plots/Zoo_FK_grid.pdf", p5, height=12, width=10)
ggsave("test_cases/plots/Zoo_FK_grid.png", p5, height=12, width=10)
...
p <- ggplot(dg) + geom_path(aes(x=P,y=g,color=sname1)) + facet_wrap(~sname2) + theme_classic()
...
p6 <- ggplot(gf_umax) + geom_tile(aes(y=prey, x=pred, fill=umax)) + 
  ...
  #scale_fill_gradientn(colors=c("#FFFFFF",rev(brewer.pal(11,"Spectral"))), na.value = 'grey50', values=scales::rescale(c(0, 0.2, 0.4, 0.8, 1, 2, 3, 4)), breaks=c(0.2, 0.4, 0.8, 1, 2, 3, 4)) + 
  scale_fill_gradientn(colors=c("#FFFFFF",rev(brewer.pal(11,"Spectral"))), na.value = 'grey50', trans="sqrt", breaks=c(0.08, 0.3, 0.7, 1.3, 2, 3, 4)) + 
  labs(x="Predators", y="Prey", fill="Maximum\ngrazing rate") + scale_x_discrete(expand=c(0,0)) + scale_y_discrete(expand=c(0,0))

ggsave("test_cases/plots/Zoo_umax_grid.pdf", p6, height=12, width=10)
ggsave("test_cases/plots/Zoo_umax_grid.png", p6, height=12, width=10)

gf_umax_cast = dcast(data=gf_umax, formula=prey~pred)

# three dimensional plot
require("plotly")
p <- plot_ly(gf_umax, x=~pred, y=~prey, z=~umax, mode='markers',
             type="scatter3d",
             marker=list(color=~umax, alpha=0.1, size=3, colorbar=list(title='Max Grazing'), colorscale='Viridis')) %>% 
  layout(xaxis=list(title="Predators"), yaxis=list(title="Prey"), dragmode="turntable")

#plot_ly(x=as.numeric(gf_umax$pred), y=as.numeric(gf_umax$prey), z=t(as.matrix(gf_umax_cast[,2:201]))) %>% add_surface()


# total predation per prey
tot_pred = data.frame(sname=s$sname, sum_umax=rev(rowSums(gf_umax_cast[,which(names(gf_umax_cast)!= "prey")], na.rm=TRUE)))
tot_pred = join(s, tot_pred)

ggplot() + geom_line(aes(x=log10(ESD_um/1000), y=sum_umax, linetype="4"), data=tot_pred) + 
  geom_line(aes(x=log10(ESD_um/1000), y=PCref_per_day, color=type, linetype="1"), data=ss) + 
  scale_linetype_manual('Value', values = c(1,2), labels=c("Max grazing", "Max Growth rate")) + 
  scale_color_manual("PFT Type", values=c("#984ea3", "#4daf4a", "#377eb8"), labels=c("Diatoms", "Diazotrophs", "Mixed Phyto")) +
  labs(y="Value") + 
  theme_classic()
...
p7 <- ggplot(fk) + 
  ...
  theme_classic() +
  #scale_color_gradient2("Zooplankton", low="#FFF7EC", high="#7F0000") +
  labs(x="Predator-prey size ratio", y="Feeding Kernel Value")

ggsave("test_cases/plots/Zoo_FK_distribution.pdf", p7, height=5, width=6)
